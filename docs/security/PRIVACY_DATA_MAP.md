...
| Meal photos | Recognition and visual history | Protected Application Support files | No in the default on-device recognizer; only with explicit future upload/retention consent | Until meal/pending scan deletion; server policy must define uploaded-photo TTL | App process; isolated recognition service when enabled | Not in JSON export; deleted with meal/pending scan/account policy |
...
| Pending hydration commands and scans | Offline durability and retry | App Group command queue; SwiftData; protected photo file | Only after configured sync/recognition and retry | Until processed, cancelled, or deleted | App, widget intent, configured recognition service | Cleared after acknowledgement/deletion |
